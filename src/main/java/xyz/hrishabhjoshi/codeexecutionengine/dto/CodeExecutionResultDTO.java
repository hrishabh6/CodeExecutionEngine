...
public class CodeExecutionResultDTO {
    ...
    /**
     * Represents the raw output collected for a single test case from the executed
     * code.
    ...
    public static class TestCaseOutput { // Now directly using the same structure as ExecutionResult.TestCaseOutput
        ...
        private String errorType; // e.g., "ArithmeticException", "ArrayIndexOutOfBoundsException", null if no
                                  // error
        private Long memoryBytes; // Peak memory usage in bytes for this test case
    }
}